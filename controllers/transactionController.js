...
async function getTransactionAndTransactionDetailById(id) {
  const result = await db.pool.query({
    text: "SELECT transaction.id,trx_number,total_amount, transaction_detail.description FROM transaction INNER JOIN transaction_detail ON transaction.id = transaction_detail.trx_id WHERE id = $1",
    values: [id],
  });
  return result.rows;
}
